...
async function getDbProject(id) {
  try {
...
export default async function CaseStudyPage({ params }) {
  ...
  if (UUID_RE.test(slug)) {
    ...
    return (
      <>
        <PageHero eyebrow={project.category} title={project.title} subtitle={project.result} />
            ...
            {project.cover_image ? (
              <Reveal>
                ...
                <img
                  ...
                  alt={project.title}
                  className="w-full rounded-2xl object-cover max-h-[480px] mb-12"
                />
              </Reveal>
            ) : (
              <Reveal>
                <div className={`w-full h-64 rounded-2xl bg-gradient-to-br ${grad} flex items-center justify-center mb-12`}>
                  <Icon name={project.icon || "Star"} className="w-20 h-20 text-white/80" />
                </div>
              </Reveal>
            )}
        ...
        <CTASection
          ...
          subtitle="Let's build something this impressive for your brand."
