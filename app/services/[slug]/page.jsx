...
export default async function ServiceDetailPage({ params }) {
  ...
  // Fetch per-service pricing; fall back to static tiers
  ...
  return (
    <>
      <PageHero eyebrow={service.title} title={service.h1} subtitle={service.sub}>
              ...
              {service.process.map((step, i) => (
                <Reveal key={i} delay={i * 0.08}>
                  <div className="surface-card p-5 text-center h-full">
                    ...
                    <p className="text-white text-sm font-medium leading-snug">{step}</p>
                  </div>
                </Reveal>
              ))}
            </div>
        ...
        <div className="container-x">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 max-w-5xl mx-auto">
            {pricingTiers.map((tier, i) => (
              <Reveal key={tier.id ?? tier.name} delay={i * 0.1}>
                <div
                ...
                >
                  ...
                  <p className="gradient-text font-heading font-extrabold text-2xl mb-1">
                    {tier.price || "Contact for Price"}
                  </p>
                  {tier.billing && (
                    <p className="text-text-secondary text-sm mb-5">{tier.billing}</p>
                  )}
                  {!tier.billing && <div className="mb-5" />}
